...
`.github/workflows/auto-version.yml` auto-versions on **every push** (skipping
the bot's own `chore(release):` commits and anything marked `[skip ci]`). The
bump follows Conventional Commits: `feat:` → minor, `fix:`/other → patch,
`!`/`BREAKING CHANGE` → major. It builds a **release APK** (generating the
`android/` scaffold + Hive adapters in CI first), then bumps `pubspec.yaml`,
tags `vX.Y.Z`, and publishes a GitHub Release with the APK attached. The build
runs *before* tagging, so a failed build cuts no release. Its own release
commits are prefixed `chore(release):` + `[skip ci]`.

To commit without cutting a release, include `[skip ci]` in the message.
